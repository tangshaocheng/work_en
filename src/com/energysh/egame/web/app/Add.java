...
public class Add extends BaseController {
	...
	public ModelAndView excute() {
		try {
			Map<String, String> rmap = this.getAppService().add(UploadHepler.saveFileAndGetPara(this.getRequest(), Constants.FUN_APP));
			PrintWriter printWriter = this.getResponse().getWriter();
			printWriter.print("<script type='text/javascript'>parent.backAdd('" + rmap.get("info") + "');</script>");
			printWriter.close();
		} catch (Exception e) {
